...
After opening a project, click on the **Readability Scores** icon on the project sidebar. There will be up to four lists available: **Raw Scores**, **Score Summary**, **Grade Score Summary (x Document)**, and **Cloze Score Summary (x Document)**.
...
To remove any test in this list from the project, select it and press `CTRL+DELETE` (`⌘+DELETE` on macOS) on your keyboard.
...
A histogram will be created for each test. For grade level tests, the bins (i.e., bars) will represent either a grade or range of grades. The height of each bar will reflect the number of documents that scored within the bar's range. In the histogram below, we can see that 21 documents scored within the 8th-9th grade range for the [Flesch-Kincaid](#flesch-kincaid-test) test:

![](Images/histogrades.png)

We interpret this first by looking at the highest bar. The label above it displays “21,” which is the number of documents in that category. Now, to see the category, we look at the x-axis labels below the bar. Because we are using [cutpoints](#options-histograms) here, there will be two labels to review. The “7” on the left side of the bar indicates the end of the **previous** bar's range. The “9” on the right side of the bar indicates the end of this bar's range. This means that this bar includes scores > 7 and ≤ 9. Because we are [rounding](#options-histograms) our scores down (the default), scores such as 7.7 will be rounded down and fall into the previous bin (i.e., bar). Scores between 8.0 and 9.9 will fall into this bin; therefore, this bar will represent the 8th-9th grade range.
...
Continuing on, this batch has quite a few difficult documents. There are 21 documents in the 8th-9th grade range, 22 documents in the 10th-11th grade range, etc. In contrast, there are few easy documents, with only 2 in the 6th-7th grade range and only 1 in the 2nd-3rd and 4th-5th ranges. From this, we can surmise that the readability of this batch needs improving.
...
If we begin to revise these documents and reload the project (`F5` on your keyboard), then the test scores will show improvement in these graphs:
...
Note that the boxes are somewhat small and range from the 7th to 12th grade. This means that the middle half of our documents fit nicely within the junior high to high school range.
...
To remove any document in this list from the project, select it and press `CTRL+DELETE` (`⌘+DELETE` on macOS) on your keyboard.
