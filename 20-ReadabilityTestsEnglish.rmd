...
For this formula, a study involving 2,600 students ranging from 4th-12th grade (*Development* 97) was conducted. The students completed 330 110-word cloze passages, and their scores were correlated against numerous linguistic factors from the passages. After a series of factor analyses and multiple regressions, a series of formulas were created to predict cloze and grade-level scores for a passage. These formulas were then cross validated against 20 250-300 word passages (*Development* 105). These formulas correlated with between *r²* = 0.81 to 0.83 with the original test data and between *r²* = 0.92 to 0.93 with the cross validation data.

Of these formulas, the predicted cloze formula is as follows:

```{r, eval=F, echo=T}
C = .886593 - .083640*(L/W) + .161911*(D/W)³ -  
     .021401*(W/S) + .000577*(W/S)² - .000005*(W/S)³
```
...
```{r, eval=F, echo=T}
G = 3.761864 + 1.053153*(L/W) - 2.138595*(D/W)³ +  
     .152832*(W/S) - .002077*(W/S)²
```
...
This test is designed for 100-word samples, with 3 samples being recommended for best results. Note that **Readability Studio** analyzes your entire document and does not use subsamples for its test calculations. If a formula requires a subsample of a specific size, then normalization is used.
...
This test was trained against 383 selections from the 1950 edition of McCall-Crabbs *Standard Test Lessons in Reading* (McCall 1950). For validation, the derived formula had an *r²* = 0.575, similar to **Farr-Jenkins-Paterson's** *r²* = 0.553. This regression equation is as follows:
...
| G   | Grade level |
| W   | Number of words\* |
| RP  | Number of characters, in this context being letters, numbers, and punctuation (except for sentence-ending punctuation) |
| S   | Number of sentences |
...
This test was trained against 376 passages from *McCall-Crabbs Standard Test Lessons in Reading* (1926 ed.). For validation, the derived formula had an *r²* = 0.575, similar to **Farr-Jenkins-Paterson's** *r²* = 0.553. This regression equation is as follows:
...
Refer to [Danielson-Bryan 1](#danielson-bryan-1) for an explanation of why number of words is used, instead of number of spaces between words.
...
```{r, eval=F, echo=T}
U = 100 - 100*(.886593 - .083640*(L/W) + .161911*(D/W)³ -  
     .021401*(W/S) + .000577*(W/S)² - .000005*(W/S)³)
```
...
Note that this test sounds out numerals' digits when syllabizing. For example, “1918” will be counted as four syllables (“one”-“nine”-“one”-“eight”). This behavior can be adjusted from [Readability Options](#readability-test-options) if you prefer a different numeral syllabizing method.

::: {.note}

There is a simplified version of this formula which uses lower precision. It was originally offered for making a manual calculation of this test easier. It is recommended to use the higher-precision formula, but the simplified version is also available in the program.
:::
...
```{r, eval=F, echo=T}
I = ROUND(206.835 - (84.6*(B/W)) - (1.015*(W/S)))
```
...
| Predicted Raw Score | Readability Score |
| :-- | :-- |
| 1.1 | 1.0 |
| 1.2 | 1.0 |
| 1.3 | 1.0 |
...
\* Readability Scores above 8.5 were derived by extrapolation (21; Chapter 3)
...
Note that numerals and proper nouns are always treated as easy words for this test, regardless of their syllable count.
...
-   recalcuated Dale-Chall: *r² = 0.5092*
-   recalcuated Flesch: *r² = 0.4034*
-   recalcuated Gunning Fog: *r² = 0.3440*
-   recalcuated Far-Jenkins-Paterson: *r² = 0.3407*
...
```{r, eval=F, echo=T}
G = 3.0680 + (.0877*(W/S)) + (.0984*((C/W)*100))
```
...
```{r, echo=F, fig.cap='Raygor Estimate Graph', fig.align='center'}
...
This test was trained on 390 passages from the 1961 edition of *McCall-Crabbs Standard Tests Lesssons in Reading*. The following regression equation was derived with a correlation of *r* = 0.985:
...
The **Spache Revised** test (Spache, *Good* 195-2003) calculates the grade level of a document based on sentence length and number of unique unfamiliar words. This test uses a list of 1041 common words that are known to younger readers (4th grade and below). Any word that does not appear on this list is considered unfamiliar. The following are also considered familiar:
...
**Spache Revised** is generally used for primary age (i.e., Kindergarten to 7th grade) readers to help classify school textbooks and literature. This differs from [New Dale-Chall](#dale-chall-test), which is better meant for secondary age readers.
...
Stocker's Catholic supplement (87-89) is an extension to the [Dale-Chall](#dale-chall-test) word list designed for Catholic students. Based on a 1971 study of 4th-grade Catholic students\*, this list represents over 200 words (not on the standard **Dale-Chall** list) familiar to 80% of them.
