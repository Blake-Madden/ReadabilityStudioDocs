...
```{r, eval=F, echo=T}
C = .886593 - .083640*(R/W) + .161911*(D/W)³ -  
...
```{r, eval=F, echo=T}
G = 3.761864 + 1.053153*(R/W) - 2.138595*(D/W)³ +  
...
```{r, eval=F, echo=T}
U = 100 - 100*(.886593 - .083640*(R/W) + .161911*(D/W)³ -  
     .021401*(W/S) + .000577*(W/S)² - .000005*(W/S)³)
```
...
- recalculated Dale-Chall: *r² = 0.5092*
- recalculated Flesch: *r² = 0.4034*
- recalculated Gunning Fog: *r² = 0.3440*
- recalculated Far-Jenkins-Paterson: *r² = 0.3407*
...
The **Spache Revised** test (Spache, *Good* 195-2003) calculates the grade level of a document based on sentence length and number of unique unfamiliar words. This test uses a list of 1041 common words that are known to younger readers (4^th^ grade and below). Any word that does not appear on this list is considered unfamiliar. The following are also considered familiar:
...
Stocker's Catholic supplement (87-89) is an extension to the [Dale-Chall](#dale-chall-test) word list designed for Catholic students. Based on a 1971 study of 4^th^-grade Catholic students\*, this list represents over 200 words (not on the standard **Dale-Chall** list) familiar to 80% of them.
