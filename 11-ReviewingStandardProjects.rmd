...
To remove any test from this list, select it and press `r keys("CTRL")`+`r keys("DELETE")` (`r macOS_cmd_key()`+`r keys("DELETE")` on macOS) on your keyboard. To edit a [custom test](#creating-custom-test), double click on it in this list. To view the help for a standard test, double click on it in this list.

There will also be graphs in this section if you had included the [Fry](#reviewing-readability-graphs) , [Gilliam-Peña-Mountain](#gilliam-pena-mountain-fry-graph)\index{Gilliam-Peña-Mountain}, [Raygor](#reviewing-readability-graphs), [*Flesch*](#flesch-test), or [FRASE](#frase)\index{FRASE} tests. Refer to \@ref(reviewing-readability-graphs) for further information.

## Readability Graphs {#reviewing-readability-graphs}

There are numerous readability tests that are graphically plotted, including [Fry](#fry-test), [Raygor](#raygor-test), [*Flesch*](#flesch-test), [Lix](#lix-test), [Gilliam-Peña-Mountain](#gilliam-pena-mountain-fry-graph), [FRASE](#frase), [Crawford](#crawford)\index{Crawford}, and [*Schwartz*](#scwartz). These graphs consist of either grade-level or reading-level regions that a document may fall within. They also include indicators for the most influential factor for the document's reported level (i.e., word difficulty vs. sentence difficulty).
...
If the data point does not fall inside of a slice then the test will fail. For example, the Raygor\index{Raygor} graph below indicates that the document's overly-long words caused the test to fail:
...
The Crawford\index{Crawford} graph is also similar to Fry in terms of factors used. However, it differs from most other graphs in that it uses the interior of the plot to find its sentence-count factor (instead of an axis). Below is an example of a *Crawford* graph:
...
```{r germanLixGauge, echo=F, fig.cap='German Lix Gauge', fig.align='center'}
...
::: {.center data-latex=""}
![](Images/StandardProjectGoals.png){width=75%}
:::
...
- Number and percentage of total (and unique) hard [SMOG](#smog-test)\index{SMOG} words. This statistic is only available if SMOG is included in the project.
...
**Dale-Chall (Unfamiliar)**: This text report contains the document with all the [*Dale-Chall*](#dale-chall-test) unfamiliar words highlighted. The *Dale-Chall* word list is a collection of words that are considered to be familiar to most fourth grade readers. Words in the document that are not found on this list may be less familiar to younger readers and hence more difficult. This window is only available if a *Dale-Chall* related test is included in the project.

**Spache (Unfamiliar)**: This text report contains the document with all the [*Spache*](#spache-test) unfamiliar words highlighted. The *Spache* word list is a collection of words that are considered to be familiar to most primary-age readers. Words in the document that are not found on this list may be less familiar to younger readers and hence more difficult. This window is only available if the *Spache* test is included in the project.

**Harris-Jacobson (Unfamiliar)**: This text report contains the document with all the [*Harris-Jacobson*](#harris-jacobson) unfamiliar words highlighted. The *Harris-Jacobson* word list is a collection of words that are considered to be familiar to most primary-age readers. Words in the document that are not found on this list may be less familiar to younger readers and hence more difficult. This window is only available if the *Harris-Jacobson* test is included in the project.
