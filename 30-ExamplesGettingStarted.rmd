...
Continuing from the [previous example](#book-example)(see \@ref(book-example)), select the top item in the hard word list. Now scroll down to the last word that has five syllables, hold down the `r keys("SHIFT")` key, and select this word. Now you will have all the words with five or more syllables selected. If there are any words that you want to add or remove from your selection, hold down the `r keys("CTRL")` key and click on them. Your word list will look like this:
