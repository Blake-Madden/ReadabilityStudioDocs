...
This will open the **Select Document to Analyze** dialog. Choose the file that you want to analyze and click the `r keyboardButton("Open")` button to accept.
...
Now that you have selected the words that you want to save, click the `r keyboardButton("Save")` button on the **Home** tab.
...
kableExtra::kbl(x=words, caption="Example of exported words",
                booktabs=T, centering=T, col.names="",
                linesep="") %>%
...
```{block, type='notesection'}
The original and revised versions of this recipe are available from the **Example Documents** menu. From the **Help** tab, click the `r keyboardButton("Example Documents")` button and then select either **Cocoa Desserts** or **Cocoa Desserts (Revised)** from the menu. When prompted about how to open the document, select **Create a new project**.
```
...
Now that we have found the difficult sentences, let us begin splitting them up so that they are easier to read. From the **Home** tab, click the `r keyboardButton("Edit Document")` button to open the recipes file for editing. Find one of the long sentences and review it. For example, consider this sentence:
...
You can click the `r keyboardButton("Reload")` button at anytime to reanalyze the source document as you edit it externally. Also, whenever you open this project, it will automatically reload the source document (in this case the recipe file).
```
...
First, click on the `r keyboardButton("New")` button (on the **Home** tab). The **New Project Wizard** dialog will appear, as shown below:
...
If you wish to include incomplete sentences in the analysis, click the `r keyboardButton("Properties")` button on the **Home** tab to display the **Project Properties** dialog. Next, click the **Document Indexing** icon, and select the **Do not exclude any text** option.
...
The first step is to open the example file **Features**. From the **Help** tab, click the `r keyboardButton("Example Documents")` button and then select **Features** from the menu. Next, when prompted about how to open the document, select **Create a new project**.
...
The first step is to open the example file **Chocolate Eclairs**. From the **Help** tab on the ribbon, click the `r keyboardButton("Example Documents")` button and then select **Chocolate Eclairs** from the menu. (When prompted about how to open the document, select **Create a new project**.) When the **New Project Wizard** appears, leave the defaults and click `r keyboardButton("Finish")`.
...
Next, we will want to apply a light color to the plot area. Click the `r keyboardButton("Background")` button and select **Plot Area Color...** from the menu and select a new color. Then, select **Opacity...** from this menu and set it to 40. Click `r keyboardButton("OK")`.

Now the plot area of the graph will have a light blue applied to it:

![](Images/ExamplePlotBackgroundColorChanged.png)

Next, let's put a [watermark](#watermark-and-logo) across our graph. Click the `r keyboardButton("Watermark")` button in the **Edit** section. Enter the following into the **Watermark** dialog and click `r keyboardButton("OK")`:
...
Now let's put a company logo on our graph. Click the `r keyboardButton("Logo")` button in the **Edit** section. Select an image and click `r keyboardButton("OK")`.

Now this image will be shown in the bottom right corner of the graph:

![](Images/ExampleLogoChanged.png)

Next, we can change the direction of the bars. Click the `r keyboardButton("Orientation")` button in the **Edit** section. Select **Vertical** from the menu.

Now the bars will be arranged vertically:

![](Images/ExampleBarChartOrientationChanged.png)

Finally, let's change how the bars are drawn. Click the `r keyboardButton("Bar Style")` button in the **Edit** section and select **Color fade, bottom to top**.
