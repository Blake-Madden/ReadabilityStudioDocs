...
When reviewing a document, it is usually preferable to ignore certain items, such as headers and citations (refer to ch. \@ref(how-text-is-excluded) for more information). In this example, we will discuss the various methods for excluding these items.

The first step is to open the example file **Danielson-Bryan**. From the **Help** tab, click the `r keyboardButton("Example Documents")` button and then select **Danielson-Bryan** from the menu. (When prompted about how to open the document, select **Create a new project**.) When the **New Project Wizard** appears, leave the defaults and click `r keyboardButton("Finish")`. A warning about an incomplete sentence being included because of its length will appear:
...
Once the project is loaded, on the **Home** tab click the `r keyboardButton("Properties")` button under the **Project** section. On the **Properties** dialog, click **Analysis** on the sidebar and select **Document Indexing**. Uncheck all the options shown below and click `r keyboardButton("OK")`:
...
Separate instances of “sentence” and “length” will still be included, they will only be excluded if they appear together to form the phrase “sentence length.” Refer to ch. \@ref(document-analysis) for more on how custom word/phrase exclusion works.
...
The first step is to open the example file **Press Release**. From the **Help** tab, click the `r keyboardButton("Example Documents")` button and then select **Press Release** from the menu. (When prompted about how to open the document, select **Create a new project**.) When the **New Project Wizard** appears, leave the defaults and click `r keyboardButton("Finish")`.
...
[Test bundles](#test-bundles) are a convenient way to add multiple tests to a project at once. In this example, we will create a test bundle named “4F” which includes the [**F**ry](#fry-test), [**F**ORCAST](#forcast-test), [**F**og](#gunning-fog-test), and [**F**lesch-Kincaid](#flesch-kincaid-test) tests. The first step is to select the **Readability** tab, click the `r keyboardButton("Bundles")` button, and select **Add...** from the menu. This will open the **Add Test Bundle** dialog:
...
To use the bundle, first open any project. Next, select the **Readability** tab, click the `r keyboardButton("Bundles")` button, and select **4F** from the menu. Now the program will replace all tests within the project with the tests from the “4F” bundle.