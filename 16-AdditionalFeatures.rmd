...
Note that verbose logging can also be enabled by holding down the `r keys("SHIFT")` button on your keyboard while the program opens.
...
The log's level of detail is controlled by either the [command line](#command-line-options) or by holding down the `r keys("SHIFT")` button on your keyword while the program starts. (Holding `r keys("SHIFT")` instructs the program to use maximum, verbose logging.)
