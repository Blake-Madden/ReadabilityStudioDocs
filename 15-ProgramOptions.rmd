...
`r BeginNoteSection()`
`r MarkdownSectionFormatLabel("The options **Ignore blank lines** and **Ignore indenting** are disabled if **Line ends always begin a new paragraph** is selected. In this case, line feeds will always indicate the end of a paragraph, whether or not it is followed by blank or indented lines.")`
`r EndSection()`
...
`r BeginNoteSection()`
`r MarkdownSectionFormatLabel("Lines of text that resemble equations will continue to be marked as incomplete. Also, if **Aggressively exclude** is enabled, then list items with more words than this threshold will also continue to be excluded.")`
`r EndSection()`
...
`r BeginNoteSection()`
`r MarkdownSectionFormatLabel("These options are not applicable if **Also exclude numerals** is checked.")`
`r EndSection()`
...
`r BeginNoteSection()`
`r MarkdownSectionFormatLabel("These options affect both the **New Dale-Chall** and **PSK Dale-Chall** tests. The **text exclusion** option will also affect any custom test using the following functions:

\x60CustomNewDaleChall()\x60
\x60UnfamiliarDaleChallWordCount()\x60
\x60UniqueUnfamiliarDaleChallWordCount()\x60
\x60FamiliarDaleChallWordCount()\x60")`
`r EndSection()`
...
`r BeginNoteSection()`
`r MarkdownSectionFormatLabel("The **text exclusion** option will affect any custom test using the following functions:

\x60CustomHarrisJacobson()\x60
\x60UnfamiliarHarrisJacobsonWordCount()\x60
\x60UniqueUnfamiliarHarrisJacobsonWordCount()\x60
\x60FamiliarHarrisJacobsonWordCount()\x60")`
`r EndSection()`
