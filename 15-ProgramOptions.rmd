...
Note that if you are not excluding any text, this option will still take effect for tests which explicitly exclude headers (e.g., Harris-Jacobson\index{Harris-Jacobson Wide Range!header exclusion}).
...
- `r keys("CTRL")`+`r keys("INSERT")` (`r macOS_cmd_key()`+`r keys("INSERT")` on macOS) —adds a new item to the list.
...
**Include Catholic Supplement**: Check this option to include [Stocker's supplementary word list](#stocker-catholic-supplement)\index{Stocker's Catholic Supplement} for Catholic students.
...
**Exclude incomplete sentences, except headings**: Select this option to have Harris-Jacobson\index{Harris-Jacobson Wide Range!header exclusion!customizing} include headers and subheaders in its analysis, but exclude lists and tables.
...
- Selection labels on the Fry\index{Fry}, *Raygor*, and Gilliam-Peña-Mountain\index{Gilliam-Peña-Mountain} graphs.
...
**Harris-Jacobson**: Check this to include the Harris-Jacobson unfamiliar words list in the results. (If Harris-Jacobson\index{Harris-Jacobson Wide Range} is included in the project).
